...
export default function EventShareMenu({ event, className = '' }) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const rootRef = useRef(null);
  const menuId = useId();

  const shareUrl = experienceShareUrl(event);
  const text = experienceShareText(event);
  ...
  return (
    <div className={`pc-share exp-share${open ? ' is-open' : ''} ${className}`.trim()} ref={rootRef}>
      <button
        type="button"
        className="pc-share-btn exp-share-btn"
        aria-expanded={open}
      ...
      {open ? (
        <div className="pc-share-menu exp-share-menu" id={menuId} role="menu">
