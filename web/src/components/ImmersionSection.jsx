...
export default function ImmersionSection() {
  ...
  const mosaic = EXPERIENCES.filter((e) => e.id !== active.id);
  ...
  return (
    ...
    >
      ...
      <div className="container immersion-inner">
        ...
        <div className="immersion-mosaic" data-reveal="up" data-delay="3">
          {mosaic.map((exp) => (
            <button
              key={exp.id}
              type="button"
              className={`immersion-tile immersion-tile--${exp.size}`}
              style={{ backgroundImage: `url('${exp.img}')` }}
              onMouseEnter={() => setActiveId(exp.id)}
              onFocus={() => setActiveId(exp.id)}
              onClick={() => setActiveId(exp.id)}
